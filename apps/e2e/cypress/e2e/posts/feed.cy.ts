...
describe("GET /posts/feed", () => {
  ...
  it("returns 200 with a paginated response shape", () => {
    cy.request(`${API_BASE_URL}/posts/feed`).then((res) => {
  ...
  it("enriches posts with author name and id", () => {
    cy.request(`${API_BASE_URL}/posts/feed?authorId=${author.id}`).then(
      (res) => {
  ...
  it("respects the limit query param", () => {
    cy.request(`${API_BASE_URL}/posts/feed?limit=1`).then((res) => {
